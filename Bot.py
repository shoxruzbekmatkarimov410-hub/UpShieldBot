...
async def cmd_start(message: types.Message):
    ...
    welcome_text = (
        ...
        "🤖 Men — " + BOT_USERNAME + " universal xavfsizlik, monitoring va yordamchi botiman.\n\n"
        "📌 Botning barcha imkoniyatlari:\n"
        "• Uptime Monitor & Saytlar\n"
        "• 24/7 Bot Hosting\n"
        "• Phishing tekshiruvi\n"
        "• Havolalarni qisqartirish\n"
        "• Port Scanner & Subdomain\n"
        "• AI Yordamchi & PDF Hisobotlar\n\n"
...
async def language_chosen(callback: types.CallbackQuery):
    ...
    if lang == "uz":
        text = "✅ O'zbek tili tanlandi! " + BOT_USERNAME + " boshqaruv paneliga xush kelibsiz."
    elif lang == "ru":
...
async def ask_for_bot_token(message: types.Message):
    ...
    if not is_prem and len(user_info.get("bots", {})) >= 1:
        if current_time - last_used < COOLDOWN_TIME:
            ...
            await message.answer("⏳ Bepul limit vaqtincha tugdi! " + str(timeLeft) + " soatdan keyin ochiladi.\n\nPremium oling:", reply_markup=get_premium_menu())
            return

    await message.answer("🤖 Botingizni 24/7 qilish uchun @BotFather'dan olgan Tokeningizni yuboring:\n\n(" + BOT_USERNAME + ")")
...
async def register_user_bot(message: types.Message):
    ...
    try:
        ...
        if user_token not in USER_DATA[user_id]["bots"]:
            ...
            await message.answer("✅ @" + str(bot_info.username) + " ulandi va 24/7 rejimida ishga tushdi! 🚀")
        else:
...
async def add_url_prompt(message: types.Message):
    ...
    if not is_prem and len(user_info.get("urls", [])) >= 1:
        if current_time - last_used < COOLDOWN_TIME:
            timeLeft = int((COOLDOWN_TIME - (current_time - last_used)) / 3600)
            await message.answer("⏳ Bepul URL limiti tugdi! " + str(timeLeft) + " soatdan keyin ochiladi.", reply_markup=get_premium_menu())
            return
...
async def save_url(message: types.Message):
    ...
    if not is_prem and len(USER_DATA[user_id]["urls"]) >= 1:
        ...
        await message.answer("❌ Bepul URL limiti tugdi! 12 soatlik cheklov ishga tushdi.")
        return
    ...
    if url not in USER_DATA[user_id]["urls"]:
        ...
        if not is_prem:
            USER_LIMITS.setdefault(user_id, {})["url_time"] = time.time()
        await message.answer("✅ Sayt qo'shildi va 24/7 tekshiruvga olindi:\n" + url)
    else:
...
async def bonus_pdf_cb(callback: types.CallbackQuery):
    ...
    if not USER_DATA.get(user_id, {}).get("is_premium", False):
        ...
        return
    await callback.message.answer("📊 PDF Hisobot tayyorlanmoqda va yuborildi ✅")

@dp.callback_query(F.data == "bonus_priority")
async def bonus_priority_cb(callback: types.CallbackQuery):
    await callback.answer("⚡️ VIP Prioritet rejimi yoqilgan!", show_alert=True)

@dp.message(F.text == "🛡 Xavfsizlik Markazi")
async def security_menu(message: types.Message):
    await message.answer("🛡 Xavfsizlik markaziga xush kelibsiz!", reply_markup=get_security_menu())

@dp.callback_query(F.data == "sec_biznes")
async def sec_biznes_cb(callback: types.CallbackQuery):
    await callback.answer()
    await callback.message.edit_text("🏢 Biznes va Oddiy foydalanuvchilar uchun:", reply_markup=get_biznes_security_menu())

@dp.callback_query(F.data == "sec_dev")
async def sec_dev_cb(callback: types.CallbackQuery):
    await callback.answer()
    await callback.message.edit_text("💻 Dasturchilar uchun xavfsizlik:", reply_markup=get_dev_security_menu())

@dp.callback_query(F.data == "sec_back")
async def sec_back_cb(callback: types.CallbackQuery):
    await callback.answer()
    await callback.message.edit_text("🛡 Xavfsizlik markaziga xush kelibsiz!", reply_markup=get_security_menu())
...
async def process_security_check(callback: types.CallbackQuery):
    ...
    if check_type in {"sec_ports", "sec_ssl", "sec_subdomain"} and not is_prem:
        await callback.answer("💎 Faqat Premium foydalanuvchilar uchun!", show_alert=True)
        await callback.message.answer("❌ Bu tahlil faqat Premium tarifda ishlaydi.", reply_markup=get_premium_menu())
        return
    ...
    if check_type == "sec_check_site":
        await msg.edit_text("🛡 Sayt xavfsizlik tahlili: SSL/TLS Ishonchli (A+), DDoS himoyasi faol ✅")
    elif check_type == "sec_check_bot":
        await msg.edit_text("🤖 Bot holati: Token xavfsiz, 24/7 ishlayapti ✅")
    elif check_type == "sec_ports":
        await msg.edit_text("🔍 Port Skaner: Port 80, 443 ochiq, SSH himoyalangan ✅")
    elif check_type == "sec_ssl":
        await msg.edit_text("🔒 SSL Tahlili: HSTS yoqilgan, muddati 320 kun ✅")
    elif check_type == "sec_subdomain":
        await msg.edit_text("⚙️ Subdomain Enumeration: API va Admin domenlar topildi ✅")
    elif check_type == "sec_phishing":
        await msg.edit_text("🕵️‍♂️ Phishing tekshiruvi: Havola xavfsiz, firibgarlik yo'q ✅")
    elif check_type == "sec_uptime":
        await msg.edit_text("🚨 Uptime: Barcha saytlar uzluksiz ishlayapti ✅")
...
async def settings_message_handler(message: types.Message):
    ...
    for url in user_info["urls"]:
        builder.button(text="🗑 Saytni o'chirish", callback_data="del_url_" + url)
        has_items = True
        
    for token, bdata in user_info["bots"].items():
        builder.button(text="🗑 Botni o'chirish", callback_data="del_bot_" + token)
        has_items = True
        
    if not has_items:
        await message.answer("⚙️ Sozlamalar paneli\n\nSizda qo'shilgan saytlar yoki botlar yo'q.")
        return

    builder.adjust(1)
    await message.answer("⚙️ O'chirish uchun quyidagilardan birini tanlang:", reply_markup=builder.as_markup())
...
async def show_premium(message: types.Message):
    premium_info = (
        ...
        "• Cheksiz saytlar uptime monitoringi\n"
        "• Cheksiz botlarni 24/7 rejimida ushlab turish\n"
        "• Port Scanner & SSL tahlillari\n"
        "• AI Yordamchi va PDF hisobotlar\n\n"
        "Tarifni tanlang:"
    )
